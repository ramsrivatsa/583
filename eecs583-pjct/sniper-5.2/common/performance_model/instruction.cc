...
SubsecondTime Instruction::getCost(Core *core) const
{
   ...
   LOG_ASSERT_ERROR(m_type < MAX_INSTRUCTION_COUNT, "Unknown instruction type: %d", m_type);
   const ComponentPeriod *componentPeriod = core->getDvfsDomain();
   unsigned long long freq = componentPeriod->getPeriodInFreqMHz();
   //std::cout << "Core freq: " << freq << std::endl;
   double period = (double)1/(double(freq*MEGA));
   double decPeriodInNS = period*NANO;
   double decPeriodInPS = decPeriodInNS*1000;
  // std::cout << "Dec period: " << decPeriodInPS << " ps" << std::endl;
   int intPeriodInNS = static_cast<int>(ceil(decPeriodInPS * (double)Instruction::m_instruction_costs[m_type]));
   SubsecondTime subsecondPeriod = SubsecondTime::PS(intPeriodInNS);

   //std::cout << "Period: " << subsecondPeriod.getPS() << " ps" << std::endl;
   //std::cout << "Inst cost: " << subsecondPeriod.getPS() << " ps" << std::endl;
   return subsecondPeriod; //static_cast<SubsecondTime>(*period) * Instruction::m_instruction_costs[m_type];
}
